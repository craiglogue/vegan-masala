...
function buildSubtitle(editorial: ReturnType<typeof getEditorialContent>) {
  ...
  return limitWords(firstSentence(raw), 16);
}
...
async function imageFrame() {
  return sharp(
    Buffer.from(`
      <svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
        <rect x="128" y="240" width="808" height="572" rx="30" ry="30"
...
async function textLayer(font: opentype.Font, title: string, hook: string, subtitle: string) {
  const titleLines = wrapWords(cleanPromoText(title), 20, 4);
  const hookLines = wrapWords(cleanPromoText(hook), 42, 2);
  const subtitleLines = wrapWords(cleanPromoText(subtitle), 54, 2);
  ...
  const subtitleBaseY = hookBaseY + hookLines.length * 28 + 14;
...
async function heroImageLayer(slug: string, type: "recipe" | "guide") {
  ...
  const roundedMask = Buffer.from(`
    <svg width="808" height="572" xmlns="http://www.w3.org/2000/svg">
      <rect x="0" y="0" width="808" height="572" rx="30" ry="30" fill="white"/>
    </svg>
  `);

  const image = await sharp(img)
    .resize(808, 572, { fit: "cover" })
  ...
  const shadow = await sharp(
    Buffer.from(`
      <svg width="836" height="600" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <filter id="shadow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="10" stdDeviation="16" flood-color="black" flood-opacity="0.20"/>
          </filter>
        </defs>
        <rect x="14" y="14" width="808" height="572" rx="30" ry="30"
...
export async function renderInstagramBySlug(slug: string) {
  ...
  if (hero.shadow) layers.push({ input: hero.shadow, left: 114, top: 226 });
if (hero.image) layers.push({ input: hero.image, left: 128, top: 240 });
  ...
  if (logo) {
    layers.push({ input: logo, left: WIDTH - 230, top: HEIGHT - 242 });
  }
